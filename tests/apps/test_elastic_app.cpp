...
template <typename T, int spatial_dim, class Mesh>
std::vector<int> get_dof_vec_from_nodes(const Mesh& mesh,
...
void test_elastic_app() {
  ...
  // Cases 1: 0-displacement on all boundary nodes with body force
  {
    auto int_fun = [](const A2D::Vec<T, Basis::spatial_dim>& xloc) {
      A2D::Vec<T, Basis::spatial_dim> int_fun;
      int_fun(0) = 0.0;
      int_fun(1) = -1.0;
      return int_fun;
    };
    ...
    for (auto nodes :
      ...
      auto t = get_dof_vec_from_nodes<T, Basis::spatial_dim>(mesh, nodes);
      bc_dof.insert(bc_dof.end(), t.begin(), t.end());
    }
    std::vector<T> sol =
        elastic.solve(bc_dof, std::vector<T>(bc_dof.size(), 0.0), {}, {});
    ToVTK<T, Mesh> vtk(mesh, "elastic_case0.vtk");
    vtk.write_mesh();
  ...
  // force
  {
    ...
    std::vector<int> left_dof = get_dof_vec_from_nodes<T, Basis::spatial_dim>(
        mesh, mesh.get_left_boundary_nodes());
    std::vector<int> right_dof = get_dof_vec_from_nodes<T, Basis::spatial_dim>(
        mesh, mesh.get_right_boundary_nodes(), {0});
