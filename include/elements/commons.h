...
static void eval_grad(const T pt[], const T dof[],
  ...
  Basis::template eval_basis_grad<T>(pt, (T*)nullptr, Nxi);
...
template <typename T, class Basis, int dim>
static void eval_grad(const T pt[], const T dof[], A2D::Vec<T, dim>& vals,
                      A2D::Mat<T, dim, Basis::spatial_dim>& grad) {
  static constexpr int spatial_dim = Basis::spatial_dim;
  static constexpr int nodes_per_element = Basis::nodes_per_element;
  ...
  T Nxi[spatial_dim * nodes_per_element];
  Basis::template eval_basis_grad<T>(pt, N, Nxi);
...
template <typename T, class Basis, int dim>
static void add_grad(const T pt[], const A2D::Vec<T, dim>& coef_vals,
                     const A2D::Mat<T, dim, Basis::spatial_dim>& coef_grad,
                     T elem_res[]) {
  static constexpr int spatial_dim = Basis::spatial_dim;
  static constexpr int nodes_per_element = Basis::nodes_per_element;
...
template <typename T, class Basis, int dim>
static void add_matrix(const T pt[],
  ...
  T Nxi[spatial_dim * nodes_per_element];
  Basis::template eval_basis_grad(pt, (T*)nullptr, Nxi);
...
template <typename T, class Basis, int dim>
static void add_matrix(const T pt[], const A2D::Mat<T, dim, dim>& coef_vals,
                       const A2D::Mat<T, dim * Basis::spatial_dim,
                                      dim * Basis::spatial_dim>& coef_grad,
                       T elem_jac[]) {
  static constexpr int spatial_dim = Basis::spatial_dim;
  ...
  T N[nodes_per_element];
  T Nxi[spatial_dim * nodes_per_element];
  Basis::template eval_basis_grad(pt, N, Nxi);
