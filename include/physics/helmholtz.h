...
class HelmholtzPhysics final : public PhysicsBase<T, spatial_dim_, 1, 1> {
 private:
  using PhysicsBase_s = PhysicsBase<T, spatial_dim_, 1, 1>;
 ...
 public:
  ...
  T energy(T weight, T x, A2D::Vec<T, spatial_dim>& _,
    ...
    return weight * detJ * (0.5 * r0square * dot + val * (val - x));
  }
  ...
  void residual(T weight, T x, A2D::Vec<T, spatial_dim>& _,
    ...
    auto stack = A2D::MakeStack(
        ...
        A2D::Eval(weight * detJ_obj *
                      (0.5 * r0square * dot_obj + u_obj * (u_obj - x_obj)),
                  output_obj));
